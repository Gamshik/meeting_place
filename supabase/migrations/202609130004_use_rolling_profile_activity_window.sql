...
begin
  ...
  activity_end_date := pg_catalog.timezone(activity_time_zone, pg_catalog.now())::date;

  if p_year = extract(year from activity_end_date) then
    activity_start_date := activity_end_date - 364;
  else
    activity_start_date := make_date(p_year, 1, 1);
    activity_end_date := make_date(p_year, 12, 31);
  end if;
  ...
  bounded_events as (
    ...
    from events event
    where pg_catalog.timezone(activity_time_zone, event.occurred_at)::date
      between activity_start_date and activity_end_date
  ),
  daily as (
    select
      event.activity_date,
...
comment on function public.get_profile_activity(uuid, integer) is
  'Returns owner-timezone activity aggregates for the previous 365 days or a selected calendar year.';
